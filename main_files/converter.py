"""
====================================================================
УМНЫЙ КОНВЕРТЕР ДОКУМЕНТОВ
====================================================================
Автоматически обрабатывает новые и изменённые файлы из папки input/
Результаты сохраняет в output/
Отслеживает уже обработанные файлы

Автор: Учебный проект Docling
Версия: 1.1 (с исправлением backend для Windows)
====================================================================
"""

import os
from dotenv import load_dotenv
# ==========================================
# 🌍 НАСТРОЙКА ПРОКСИ ИЗ .ENV
# ==========================================
# Загружаем переменные из файла .env
load_dotenv()

# Получаем значение PROXY_URL
proxy_url = os.getenv("PROXY_URL")

if proxy_url:
    print(f"⚙️  Найдена настройка прокси в .env")
    # Применяем прокси
    os.environ["HTTP_PROXY"] = proxy_url
    os.environ["HTTPS_PROXY"] = proxy_url
    print(f"   ✅ Прокси активирован")
else:
    print("ℹ️  Прокси не задан (файл .env или переменная PROXY_URL отсутствуют)")

# ==========================================

import json
import time
from pathlib import Path
from datetime import datetime



# Основные импорты Docling
from docling.document_converter import DocumentConverter, PdfFormatOption
from docling.datamodel.base_models import InputFormat
from docling.datamodel.pipeline_options import PdfPipelineOptions, TableFormerMode
# ВАЖНО: Импортируем альтернативный движок (backend), который не зависит от resources.dat
from docling.backend.pypdfium2_backend import PyPdfiumDocumentBackend


# ====================================================================
# КОНФИГУРАЦИЯ
# ====================================================================

# Папки проекта (поднимаемся на уровень выше из папки скрипта main_files)
# Если converter.py лежит в корне проекта, используйте Path("input")
# Если converter.py лежит в main_files/, используйте .parent.parent
SCRIPT_DIR = Path(__file__).parent
PROJECT_ROOT = SCRIPT_DIR.parent if SCRIPT_DIR.name == "main_files" else SCRIPT_DIR

INPUT_DIR = PROJECT_ROOT / "input"           # Папка с исходными файлами
OUTPUT_DIR = PROJECT_ROOT / "output"         # Папка для результатов
LOG_FILE = PROJECT_ROOT / ".processing_log.json"  # База обработанных файлов

# Поддерживаемые форматы
SUPPORTED_FORMATS = ['.pdf', '.docx', '.pptx', '.doc', '.xlsx', '.html']


# ====================================================================
# ФУНКЦИЯ: Загрузка базы обработанных файлов
# ====================================================================

def load_processing_log():
    """
    Загружает информацию об уже обработанных файлах из JSON
    Возвращает:
        dict: Словарь {имя_файла: {"modified": время, "output": путь}}
    """
    if LOG_FILE.exists():
        try:
            with open(LOG_FILE, 'r', encoding='utf-8') as f:
                return json.load(f)
        except json.JSONDecodeError:
            return {}
    return {}


# ====================================================================
# ФУНКЦИЯ: Сохранение базы обработанных файлов
# ====================================================================

def save_processing_log(log_data):
    """
    Сохраняет информацию об обработанных файлах в JSON
    Параметры:
        log_data (dict): Данные для сохранения
    """
    with open(LOG_FILE, 'w', encoding='utf-8') as f:
        json.dump(log_data, f, indent=2, ensure_ascii=False)


# ====================================================================
# ФУНКЦИЯ: Получение времени модификации файла
# ====================================================================

def get_file_modified_time(file_path):
    """
    Получает время последней модификации файла
    """
    return os.path.getmtime(file_path)


# ====================================================================
# ФУНКЦИЯ: Проверка нужно ли обрабатывать файл
# ====================================================================

def should_process_file(file_path, log_data):
    """
    Проверяет, нужно ли обрабатывать файл
    (новый или изменённый с момента последней обработки)
    """
    file_name = file_path.name
    current_modified = get_file_modified_time(file_path)
    
    # Если файла нет в базе - нужно обработать
    if file_name not in log_data:
        return True
    
    # Если файл изменился с момента последней обработки - нужно обработать
    last_modified = log_data[file_name].get('modified', 0)
    if current_modified > last_modified:
        return True
    
    # Файл уже обработан и не изменился
    return False


# ====================================================================
# ФУНКЦИЯ: Конвертация одного файла
# ====================================================================

def convert_file(file_path, converter):
    """
    Конвертирует один файл в Markdown
    Параметры:
        file_path (Path): Путь к исходному файлу
        converter: Объект DocumentConverter
    Возвращает:
        tuple: (успех, путь_к_результату, время_обработки)
    """
    start_time = time.time()
    
    try:
        # Конвертируем документ
        result = converter.convert(str(file_path))
        doc = result.document
        
        # Формируем имя выходного файла
        output_filename = file_path.stem + ".md"
        output_path = OUTPUT_DIR / output_filename
        
        # Сохраняем результат
        with open(output_path, "w", encoding="utf-8") as f:
            f.write(doc.export_to_markdown())
        
        elapsed_time = time.time() - start_time
        
        return True, output_path, elapsed_time
        
    except Exception as e:
        elapsed_time = time.time() - start_time
        print(f"   ❌ Ошибка: {e}")
        return False, None, elapsed_time


# ====================================================================
# ФУНКЦИЯ: Получение списка файлов для обработки
# ====================================================================

def get_files_to_process():
    """
    Сканирует папку input/ и возвращает список файлов для обработки
    """
    files_to_process = []
    
    # Проверяем, существует ли папка input
    if not INPUT_DIR.exists():
        print(f"❌ Папка {INPUT_DIR} не найдена!")
        return files_to_process
    
    # Сканируем папку
    for file_path in INPUT_DIR.iterdir():
        # Пропускаем папки
        if not file_path.is_file():
            continue
        
        # Проверяем расширение
        if file_path.suffix.lower() in SUPPORTED_FORMATS:
            files_to_process.append(file_path)
    
    return files_to_process


# ====================================================================
# ГЛАВНАЯ ФУНКЦИЯ
# ====================================================================

def main():
    """
    Главная функция: сканирует input/, обрабатывает новые/изменённые файлы
    """
    print()
    print("=" * 70)
    print("🚀 УМНЫЙ КОНВЕРТЕР ДОКУМЕНТОВ (PyPdfium Backend)")
    print("=" * 70)
    print(f"📂 Входная папка: {INPUT_DIR.absolute()}")
    print(f"📂 Выходная папка: {OUTPUT_DIR.absolute()}")
    print()
    
    # Создаём папки если их нет
    try:
        INPUT_DIR.mkdir(exist_ok=True)
        OUTPUT_DIR.mkdir(exist_ok=True)
    except OSError as e:
        print(f"❌ Ошибка при создании папок: {e}")
        return
    
    # Загружаем базу обработанных файлов
    log_data = load_processing_log()
    print(f"📋 Загружена база: {len(log_data)} обработанных файлов")
    
    # Получаем список всех файлов в input/
    all_files = get_files_to_process()
    print(f"📄 Найдено файлов в input/: {len(all_files)}")
    print()
    
    if not all_files:
        print("⚠️  Нет файлов для обработки!")
        print(f"   Положите файлы ({', '.join(SUPPORTED_FORMATS)}) в папку {INPUT_DIR}")
        return
    
    # Фильтруем: оставляем только новые и изменённые
    files_to_process = [f for f in all_files if should_process_file(f, log_data)]
    
    if not files_to_process:
        print("✅ Все файлы уже обработаны!")
        print("   Для повторной обработки измените файл или удалите .processing_log.json")
        return
    
    print(f"🔄 Нужно обработать: {len(files_to_process)} файлов")
    print()
    
    # --- НАСТРОЙКА КОНВЕРТЕРА ---
    print("📦 Инициализация конвертера...")
    
    # 1. Настройки Pipeline (что и как искать в PDF)
    pipeline_options = PdfPipelineOptions(
        do_ocr=True,               # Включить OCR (для сканированных документов)
        do_table_structure=True,   # Искать таблицы
        table_structure_options={"mode": TableFormerMode.ACCURATE}  # Точный режим для таблиц
    )

    # 2. Настройка формата PDF с использованием PyPdfium2Backend
    # Это решает проблему "filename does not exists: .../additional.dat" на Windows
    format_options = {
        InputFormat.PDF: PdfFormatOption(
            pipeline_options=pipeline_options,
            backend=PyPdfiumDocumentBackend 
        )
    }
    
    # 3. Создание объекта конвертера
    converter = DocumentConverter(format_options=format_options)
    
    print("   ✅ Готово!")
    print()
    
    # Обрабатываем каждый файл
    success_count = 0
    error_count = 0
    total_time = 0
    
    for i, file_path in enumerate(files_to_process, 1):
        print(f"[{i}/{len(files_to_process)}] 📄 {file_path.name}")
        
        # Конвертируем
        success, output_path, elapsed_time = convert_file(file_path, converter)
        total_time += elapsed_time
        
        if success:
            success_count += 1
            print(f"   ✅ Готово за {elapsed_time:.2f} сек → {output_path.name}")
            
            # Обновляем базу
            log_data[file_path.name] = {
                'modified': get_file_modified_time(file_path),
                'output': str(output_path),
                'processed_at': datetime.now().isoformat(),
                'success': True
            }
        else:
            error_count += 1
        
        print()
    
    # Сохраняем обновлённую базу
    save_processing_log(log_data)
    
    # Итоги
    print("=" * 70)
    print("📊 ИТОГИ")
    print("=" * 70)
    print(f"✅ Успешно: {success_count}")
    print(f"❌ Ошибок: {error_count}")
    print(f"⏱️  Общее время: {total_time:.2f} секунд")
    print(f"📂 Результаты: {OUTPUT_DIR.absolute()}")
    print("=" * 70)
    print()


# ====================================================================
# ТОЧКА ВХОДА
# ====================================================================

if __name__ == "__main__":
    main()
